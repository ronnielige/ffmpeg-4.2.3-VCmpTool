...
typedef struct VideoState {
    ...
    int scaleto_width;
    int scaleto_height;
    AVFrame* splitline_frame;
    char format_str[60];  // format string: wxh@fps - codecname
    double frame_rate;
    ...
    SDL_Texture *split_texture;
    TTF_Font    *font;
    int show_info_type;  // show decode information type
    SampleList  sampleHeader;
    int64_t   displayed_frames;
    int64_t   last_disp_frames_tot_bits;
    int64_t   disp_frames_tot_bits;
...
static void PushSampleList(SampleList* pHeader, SampleList* p)
{
    SampleList* pCur = pHeader;
...
static void PopSampleList(SampleList* pHeader, int64_t pts, SampleList** p)
{
    SampleList* pCur = pHeader;
    *p = NULL;
    if(pCur == NULL)
...
static void CleanSampleList(SampleList* pHeader)
{
    if(pHeader == NULL)
        return;
    while(pHeader->pNext)
    {
        SampleList* p = pHeader->pNext;
...
/*
* Calculate aux stream display rect and return texture rect for Render Copy
* Note: 1. Video Window means video content window, It's width and height is the resolution of this video, and don't change for this stream 
*       2. SDL Window means the application window to show video content, it's resulution wouhld be changed while window resize event happend
*       3. SDL Window may not same with Video Window, such as full screen play mode.
* Follow information present difference between Video Window and SDL Window, We must fit Video Window into SDL Window
* *Video Window    <--vert_split_pos
* |---------------|---------|
* |               |         |
* |               | TextRect|
* |               |         |
...
*                  src_height
*/  
static void calculate_aux_display_rect(SDL_Rect *disp_rect, SDL_Rect *text_rect, SDL_Rect *disp_split_rect, SDL_Rect *split_rect, 
                                       int vert_split_pos, int scr_xleft, int scr_ytop, int scr_width, int scr_height,
                                       int pic_width, int pic_height, AVRational pic_sar)
{
    AVRational aspect_ratio = pic_sar;
    int64_t width, height, x, y;
    int64_t vert_split_pos_disp;

    //av_log(NULL, AV_LOG_INFO, "In aux_display_rect: scr_width, scr_height %d, %d, pic_width, pic_height %d %d, pic_sar %d:%d\n", 
    //       scr_width, scr_height, pic_width, pic_height, pic_sar.den, pic_sar.num);
    if (av_cmp_q(aspect_ratio, av_make_q(0, 1)) <= 0)
        aspect_ratio = av_make_q(1, 1);
    ...
    //av_log(NULL, AV_LOG_FATAL, "width height %d %d\n", width, height);
    x = (scr_width - width) / 2;
    y = (scr_height - height) / 2;
    vert_split_pos_disp = vert_split_pos * width / pic_width;
    disp_rect->x = scr_xleft + x + vert_split_pos_disp;
    disp_rect->y = scr_ytop  + y;
    disp_rect->w = FFMAX((int)width,  1) - vert_split_pos_disp;
    disp_rect->h = FFMAX((int)height, 1);
    text_rect->x = vert_split_pos;
    text_rect->y = 0;
    text_rect->w = pic_width - vert_split_pos;
    text_rect->h = pic_height;
    disp_split_rect->x = scr_xleft + x + vert_split_pos_disp;
    disp_split_rect->y = scr_ytop  + y;
    disp_split_rect->w = 2;
    disp_split_rect->h = FFMAX((int)height, 1);
    split_rect->x = vert_split_pos;
    split_rect->y = 0;
    split_rect->w = 2;
    split_rect->h = pic_height;

    //av_log(NULL, AV_LOG_INFO, "In aux_display_rect: src_width, width %d, %d, x, y %d %d, vert_split_pos & disp %d, %d, pic_width %d\n", 
    //       scr_width, width, x, y, vert_split_pos, vert_split_pos_disp, pic_width);
    //av_log(NULL, AV_LOG_INFO, "In aux_display_rect: disp_rect %d %d %d %d, text_rect %d %d %d %d\n", 
    //       disp_rect->x, disp_rect->y, disp_rect->w, disp_rect->h, text_rect->x, text_rect->y, text_rect->w, text_rect->h);
}
...
static void init_split_frame(AVFrame* frame)
{
    int i, j;
    for(i = 0; i < frame->height; i++)
        for(j = 0; j < frame->width; j++)
            *(uint16_t *)(frame->data[0] + i * frame->linesize[0] + j) = 255;
    for (i = 0; i < frame->height / 2; i++)
        for(j = 0; j < frame->width / 2; j++)
        {
            *(frame->data[1] + i * frame->linesize[1] + j) = 128;
            *(frame->data[2] + i * frame->linesize[2] + j) = 128;
        }
}
...
static void video_image_display(VideoState *is, VideoState *aux_is)
{
    ...
    Frame *sp = NULL;
    SDL_Rect rect, aux_rect, aux_texture_rect, disp_sp_frame_rect, split_frame_rect, font_rect;
    char new_title[400], left_title[200], right_title[200];
    char left_detail[400] = "", right_detail[200] = "";
    char key_binding[200] = "";
    double elapse_time; // second 
    ...
    if(sl && is->displayed_frames && (is->displayed_frames % (int)is->frame_rate) == 0)
    {
        ...
        max_main_bitrate = max_main_bitrate < main_bitrate_interval ? main_bitrate_interval : max_main_bitrate;
    ...
    if(aux_is)
    {
        ...
        PopSampleList(&aux_is->sampleHeader, aux_vp->frame->pts, &aux_sl);
        if(aux_sl)  // not pause stat
        {
            aux_pkt_size = aux_sl->pkt_size;
            free(aux_sl);
        ...
        if (aux_sl && aux_is->displayed_frames && (aux_is->displayed_frames % (int)aux_is->frame_rate) == 0)
        {
            ...
            max_aux_bitrate = max_aux_bitrate < aux_bitrate_interval ? aux_bitrate_interval : max_aux_bitrate;
    ...
    {
        int vns = 0, vhh = 0, vmm = 0, vss = 0, vms = 0;
        if(!isnan(vp->pts)) // raw bitstream may have no pts
        {        
            vns  = (int)vp->pts;
            vhh  = vns / 3600;
            vmm  = (vns % 3600) / 60;
    ...
    if (aux_is) // blend two frame and add a split line
    {
        ...
        set_sdl_yuv_conversion_mode(aux_vp->frame);
        SDL_RenderCopyEx(renderer, aux_is->vid_texture, &aux_texture_rect, &aux_rect, 0, NULL, aux_vp->flip_v ? SDL_FLIP_VERTICAL : 0);
        // add split line
        if (upload_texture(&is->split_texture, is->splitline_frame, &is->img_convert_ctx) < 0) 
            return;
        SDL_RenderCopyEx(renderer, is->split_texture, &split_frame_rect, &disp_sp_frame_rect, 0, NULL, aux_vp->flip_v ? SDL_FLIP_VERTICAL : 0);
        set_sdl_yuv_conversion_mode(NULL);
        ...
        if (!is->paused)
        {
            if (is->vert_split_pos + is->mov_speed > is->scaleto_width || is->vert_split_pos + is->mov_speed < 0)
...
static void stream_close(VideoState *is)
{
    ...
    if (is->split_texture)
        SDL_DestroyTexture(is->split_texture);
    if (is->sub_texture)
...
static int video_open(VideoState *is, VideoState *aux_is)
{
    int w,h;
    char new_title[400];
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            time= av_gettime_relative()/1000000.0;
            //av_log(NULL, AV_LOG_FATAL, "time = %f, is->frame_timer %f, delay %f\n", time, is->frame_timer, delay);
            if (is->first_pts >= 0 && time < is->frame_timer + delay) { // means current frame arrived too early, need to wait, return remaining_time to wait
                //av_log(NULL, AV_LOG_FATAL, "frame arrived too early: time %f, is->frame_timer + delay %f, diff %f\n", time , is->frame_timer + delay, is->frame_timer + delay - time);
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        // frame may need scale: 20200511: deprecated, use SDL_RenderCopyEx to do scale now
        if(0 && scaleFrame == NULL && (is->viddec_width != is->scaleto_width || is->viddec_height != is->scaleto_height))
        {
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        is->frame_rate = (double)frame_rate.num / frame_rate.den;
        sprintf(is->format_str, "(%dx%d@%5.2ffps-%s %lld kbps)", is->viddec_width, is->viddec_height, is->frame_rate, codec->name, is->ic->bit_rate / 1000);
...
//      of the seek_pos/seek_rel variables
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            ...
            SampleList* p = (SampleList*)malloc(sizeof(SampleList));
            p->pts = pkt_ts;
            p->pkt_size = pkt->size;
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    is->sampleHeader.pts = -111111;
    is->show_info_type = SHOW_HINT;
    if (!is->read_tid) {
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_PAGEDOWN:
                ...
                seek_chapter(cur_stream, -1);
                break;
            case SDLK_TAB: // Tab key is used to show decode informations
                cur_stream->show_info_type = ((cur_stream->show_info_type + 1) % SHOW_TOTAL);
                if(cur_stream->show_info_type > 0)
                    cur_stream->show_info_type = SHOW_DETAIL;
                video_display(cur_stream, auxlilary_stream);
                break;
            case SDLK_LEFT:
        ...
        case SDL_WINDOWEVENT:
            switch (event.window.event) {
                case SDL_WINDOWEVENT_SIZE_CHANGED:
                    ...
                    if (auxlilary_stream)
                    {
                        auxlilary_stream->width = cur_stream->width;
                        auxlilary_stream->height = cur_stream->height;
                        if (auxlilary_stream->vis_texture)
                        {
                            SDL_DestroyTexture(auxlilary_stream->vis_texture);
                            auxlilary_stream->vis_texture = NULL;
                        }
                    }
...
static void show_usage(void)
{
    ...
    printf("       F2                  step forward one frame for the right file\n");
    printf("       right mouse click   seek to percentage in file accord to mouse position\n");
    printf("       left/right          seek backward/forward 10 seconds\n");
...
int main(int argc, char **argv)
{
    ...
    char font_dir[4][50] = {"C:\\Windows\\Fonts\\msyh.ttf", 
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        if(is->font != NULL)
            ...
            av_log(NULL, AV_LOG_FATAL, "Open font %s failed\n", font_dir[i]);
    }

    if (input_filename[1])
    {
        int max_width, max_height;
        av_usleep((int64_t)(0.200 * 1000000.0)); // sleep 200ms wait first stream finish init

        //av_log(NULL, AV_LOG_FATAL, "main stream width height %d %d\n", is->viddec_width, is->viddec_height);
        ...
        max_width  = FFMAX(is->viddec_width,  is2->viddec_width);
        max_height = FFMAX(is->viddec_height, is2->viddec_height);
        is->scaleto_width  = is2->scaleto_width  = max_width;
        is->scaleto_height = is2->scaleto_height = max_height;
        // if(is->viddec_width > is2->viddec_width) //TODO: currently, main stream resolution can't be greater than aux stream to avoid problems
        // {
        //     is2->font = is->font;
        //     swap_ptr((void**)&input_filename[0], (void**)&input_filename[1]);
        //     swap_ptr((void**)&is, (void**)&is2);
        // }
        ...
        is->splitline_frame = av_frame_alloc();
        if (is->splitline_frame)
        {
            is->splitline_frame->format = is->viddec.avctx->pix_fmt;
            is->splitline_frame->width = is->scaleto_width;
            is->splitline_frame->height = is->scaleto_height;
